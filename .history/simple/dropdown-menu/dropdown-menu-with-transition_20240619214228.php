...
<script>
    ...
    function showNested(event, id) {
        ...
        // Determine the correct parent container to push into the navigation stack
        var parentDropdown = nestedContent.closest('.dropdown-content');
        var parentNested = nestedContent.closest('.nested-content');
        
        if (parentDropdown) {
            navigationStack.push(parentDropdown);
        } else if (parentNested) {
            navigationStack.push(parentNested);
        }
    }
