...
function goBack(event) {
    ...
    if (currentContent) {
        ...
        currentContent.addEventListener('animationend', function () {
            ...
            // Check if there's a parent dropdown in the stack
            var parentDropdown = navigationStack.pop();
            if (parentDropdown) {
                console.log("Previous dropdown found in stack:", parentDropdown.id);
                parentDropdown.classList.add("enter-from-left", "show");

                // Remove the enter-from-left class after the animation ends
                parentDropdown.addEventListener('animationend', function() {
                    console.log("Animation ended for:", parentDropdown.id);
                    parentDropdown.classList.remove("enter-from-left");
                }, { once: true });
            } else {
                // If there's no parent dropdown in the stack, show the root dropdown
                var rootDropdown = document.querySelector('.dropdown-content');
                if (rootDropdown) {
                    console.log("No previous dropdown found in stack. Showing root dropdown:", rootDropdown.id);
