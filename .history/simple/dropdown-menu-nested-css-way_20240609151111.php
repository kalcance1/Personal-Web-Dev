...
function goBack(event) {
    ...
    if (currentContent) {
        ...
        console.log("Navigation stack after going back:", navigationStack.map(elem => elem.id));
    }
}
...
function proceedAfterAnimation() {
    ...
    if (previousDropdown) {
        ...
        // Find the parent dropdown of the current nested content
        var parentDropdown = navigationStack[navigationStack.length - 1];

        // If there's a parent dropdown, show it
        if (parentDropdown) {
            console.log("Parent dropdown found:", parentDropdown.id);

            // Delay showing the parent dropdown until the exit animation completes
            setTimeout(function() {
                parentDropdown.classList.add("enter-from-left", "show");

                // Remove the enter-from-left class after the animation ends
                setTimeout(function() {
                    console.log("Removing animation class for:", parentDropdown.id);
