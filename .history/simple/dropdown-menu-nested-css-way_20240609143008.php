...
if (currentContent) {
    ...
    // Define a flag to track whether the animation end event has already been handled
    var animationEnded = false;

    // Remove the 'show' class and animation class after the animation ends
    currentContent.addEventListener('animationend', function animationEndHandler() {
        // Check if the event listener has already been triggered
        if (!animationEnded) {
            console.log("Animation ended for:", currentContent.id);
            currentContent.classList.remove("show", "exit-to-right");
            // Set the flag to true to indicate that the event listener has been triggered
            animationEnded = true;
            // Remove the event listener to prevent multiple executions
            currentContent.removeEventListener('animationend', animationEndHandler);
        }
    }, { once: true });
